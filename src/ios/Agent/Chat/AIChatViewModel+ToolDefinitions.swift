...
extension AIChatViewModel {
    ...
    func makeAgentTools() -> [AgentToolDefinition] {
        ...
        tools.append(AgentToolDefinition(
            name: "ipad_executor",
            description: "Run an ordered diagnostic task on the user's own jailbroken iPad through SSH from the phone's iSH environment. The iPad executor remains localhost-only and enforces its own allowlist and audit log. Requires SSH key authentication already configured.",
            parameters: [
                "tool_title": AgentToolParam(type: .string, description: "A concise summary of the iPad task."),
                "steps": AgentToolParam(type: .string, description: "JSON array string, for example [{\"command\":\"uname -a\"}]."),
                "stop_on_error": AgentToolParam(type: .boolean, description: "Stop after the first failed step."),
                "host": AgentToolParam(type: .string, description: "iPad LAN address; defaults to 192.168.110.25."),
            ],
            required: ["tool_title", "steps"],
            propertyOrdering: ["tool_title", "steps", "stop_on_error", "host"]
        ))
