    ...
    func executeSingleToolUse(
        ...
        case "ipad_executor":
            guard let stepsJSON = toolArgs["steps"] as? String,
                  let stepsData = stepsJSON.data(using: .utf8),
                  let steps = try? JSONSerialization.jsonObject(with: stepsData) as? [[String: Any]],
                  !steps.isEmpty, JSONSerialization.isValidJSONObject(steps) else {
                toolOutput = "Error: steps must be a non-empty JSON array string."
                toolSuccess = false
                break
            }
            ...
            guard let data = try? JSONSerialization.data(withJSONObject: payload) else {
                toolOutput = "Error: invalid iPad task JSON."
                toolSuccess = false
                break
            }
            let encoded = data.base64EncodedString()
            let host = (toolArgs["host"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
            let safeHost = (host?.isEmpty == false ? host! : "192.168.110.25")
            guard safeHost.range(of: "^[A-Za-z0-9.:-]+$", options: .regularExpression) != nil else {
                toolOutput = "Error: invalid iPad host."
                toolSuccess = false
                break
            }
            let remote = "echo \(encoded) | base64 -d | /var/jb/usr/bin/python3 -c 'import sys,json,urllib.request; d=json.load(sys.stdin); b=json.dumps(d).encode(); r=urllib.request.Request(\"http://127.0.0.1:8765/v1/task\",data=b,headers={\"Content-Type\":\"application/json\"}); print(urllib.request.urlopen(r,timeout=125).read().decode())'"
            let ssh = "ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o KexAlgorithms=curve25519-sha256 -o ConnectTimeout=10 mobile@\(safeHost) \"\(remote)\""
            do {
                let result = try await executeCommand(ssh, timeout: 135) { _ in }
                toolOutput = result.output
                toolSuccess = result.exitCode == 0
            } catch {
                toolOutput = "iPad SSH executor unavailable: \(error.localizedDescription)"
                toolSuccess = false
            }
